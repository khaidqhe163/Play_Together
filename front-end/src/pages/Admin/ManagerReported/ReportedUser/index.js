...
const ReportedUser = () => {
    ...
    useEffect(() => {
        const getReports = async () => {
            try {
                const data = await axios.get("http://localhost:3008/api/report/report-player")
    ...
    return (
        <LayoutAdmin>
            <ContainerPage>
                ...
                {
                    reportId && <ReportPlayerModal lgShow={lgShow} setLgShow={setLgShow} id={reportId} reports={reports} setReports={setReports} />
                }
            </ContainerPage>
        </LayoutAdmin>
    );
